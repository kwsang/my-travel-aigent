...
def validate_itinerary_structure(itinerary: dict, risk_tolerance: str, circadian_pref: str, user_prefs: dict = None):
    ...
    # 8. Teleportation / Arrival Sanity Check
    starting_location = profile.preferences.starting_location
    ...
    if starting_location and destination:
        ...
        if current_city != dest_city:
            # Flatten the already-sorted events to evaluate them purely chronologically
            all_sorted_events = []
            for day in sorted_days:
                all_sorted_events.extend(events_by_day[day])
                
            for e in all_sorted_events:
                segment = e.get("segment")
                details = e.get("details", {})
                
                raw_event_city = details.get("city") or ""
                event_city = raw_event_city.split(',')[0].strip().lower()
                
                if not event_city:
                    if dest_city in details.get("name", "").lower():
                        event_city = dest_city
