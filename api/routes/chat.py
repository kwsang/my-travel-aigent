...
async def chat(
    ...
    try:
        ...
        async def event_generator():
            ...
            async def run_agent():
                try:
                    ...
                    async for event in runner.run_async(
                        ...
                        # Fetch and yield intermediate state after each reasoning hop directly from the database.
                        # This bypasses the ADK's session cache which only flushes at the end of the entire turn!
                        intermediate_doc = await db.itineraries.find_one({"session_id": request.session_id, "user_id": user_id})
                        if intermediate_doc:
                            intermediate_doc.pop("_id", None)
            ...
            # Start the agent processing in a separate background task
            agent_task = asyncio.create_task(run_agent(), context=contextvars.Context())
